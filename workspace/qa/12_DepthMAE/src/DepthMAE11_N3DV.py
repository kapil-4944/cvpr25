...
# MAE measure between predicted depths and depths from dense input NeRF for spiral video
# Modified from DepthMAE01_N3DV.py
...
this_metric_name = this_filename[:-5]
...
class DepthMAE:
    ...
    @staticmethod
    def compute_depth_mae(gt_depth: numpy.ndarray, eval_depth: numpy.ndarray):
        error = gt_depth.astype('float') - eval_depth.astype('float')
        mae = numpy.mean(numpy.abs(error))
        return mae

    def compute_avg_mae(self, old_data: pandas.DataFrame, dense_model_dirpath: Path, pred_train_dirpath: Path,
        ...
        for scene_name in tqdm(scene_names, leave=self.verbose_log):
            pred_video_num = 0
            gt_depth_path = dense_model_dirpath / f'{scene_name}/predicted_videos_iter{90000:06}/depth/{pred_video_num:04}_spiral01.mp4'
            gt_depth_scales_path = dense_model_dirpath / f'{scene_name}/predicted_videos_iter{90000:06}/depth/depth_scales_spiral01_VideoWise.csv'
            pred_depth_path = pred_train_dirpath / f'{scene_name}/predicted_videos_iter{iter_num:06}/depth/{pred_video_num:04}_spiral01.mp4'
            pred_depth_scales_path = pred_train_dirpath / f'{scene_name}/predicted_videos_iter{iter_num:06}/depth/depth_scales_spiral01_VideoWise.csv'
            ...
            for frame_num in range(gt_depth.shape[0]):
                if old_data is not None and old_data.loc[
                    ...
                    continue
                qa_score = self.compute_depth_mae(gt_depth[frame_num], pred_depth[frame_num])
        ...
        avg_mae = numpy.mean(merged_data[this_metric_name])
        merged_data = merged_data.round({this_metric_name: 4, })
        avg_mae = numpy.round(avg_mae, 4)
        if isinstance(avg_mae, numpy.floating):
            avg_mae = avg_mae.item()
        return avg_mae, merged_data
...
def start_qa(pred_train_dirpath: Path, database_dirpath: Path, dense_model_dirpath: Path, resolution_suffix,
    ...
    mae_computer = DepthMAE(videos_data)
    ...
    for iter_num in iter_nums:
        ...
        if str(iter_num) in qa_scores:
            if this_metric_name in qa_scores[str(iter_num)]:
                avg_mae = qa_scores[str(iter_num)][this_metric_name]
                print(f'Average {this_metric_name}: {pred_train_dirpath.as_posix()} - {iter_num:06}: {avg_mae}')
                print('Running QA again.')
        else:
            qa_scores[str(iter_num)] = {}

        mae_data_path = pred_train_dirpath / f'quality_scores/iter{iter_num:06}/{this_metric_name}_FrameWise.csv'
        if mae_data_path.exists():
            mae_data = pandas.read_csv(mae_data_path)
        else:
            mae_data = None

        avg_mae, mae_data = mae_computer.compute_avg_mae(mae_data, dense_model_dirpath, pred_train_dirpath, iter_num,
                                                             resolution_suffix, downsampling_factor)
        if numpy.isfinite(avg_mae):
            avg_scores[iter_num] = avg_mae
            qa_scores[str(iter_num)][this_metric_name] = avg_mae
            print(f'Average {this_metric_name}: {pred_train_dirpath.as_posix()} - {iter_num:06}: {avg_mae}')
            with open(qa_scores_filepath.as_posix(), 'w') as qa_scores_file:
                simplejson.dump(qa_scores, qa_scores_file, indent=4)
            mae_data_path.parent.mkdir(parents=True, exist_ok=True)
            mae_data.to_csv(mae_data_path, index=False)
    return avg_scores


def demo1():
    pred_train_dirpath = Path('../../../view_synthesis/research/010_SameViewDenseFlow/runs/training/train0004')
    database_dirpath = Path('../../../../databases/N3DV/data')
    dense_model_dirpath = Path('../../../view_synthesis/research/003_Kplanes/runs/training/train0016')
