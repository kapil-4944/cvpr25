...
class VMAF:
    ...
    def compute_avg_vmaf(self, old_data: pandas.DataFrame, database_dirpath: Path, pred_train_dirpath: Path,
                         iter_num: int, camera_prefix: str, resolution_suffix: str, downsampling_factor: int):
        """
        ...
        for i, video_data in tqdm(self.videos_data.iterrows(), total=self.videos_data.shape[0], leave=self.verbose_log):
            ...
            gt_rgb_path = database_dirpath / f'all/database_data/{scene_name}/rgb{camera_prefix}{resolution_suffix}/{pred_video_num:04}.mp4'
...
def start_qa(pred_train_dirpath: Path, database_dirpath: Path, camera_suffix: str, resolution_suffix: str, downsampling_factor: int):
