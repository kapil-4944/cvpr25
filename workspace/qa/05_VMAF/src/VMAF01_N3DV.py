...
# VMAF measure between predicted videos and ground truth videos
...
class VMAF:
    ...
    def compute_avg_vmaf(self, old_data: pandas.DataFrame, database_dirpath: Path, pred_train_dirpath: Path,
        ...
        for i, video_data in tqdm(self.videos_data.iterrows(), total=self.videos_data.shape[0], leave=self.verbose_log):
            ...
            gt_rgb_path = database_dirpath / f'all/database_data/{scene_name}/rgb{resolution_suffix}/{pred_video_num:04}.mp4'
            pred_rgb_path = pred_train_dirpath / f'{scene_name}/predicted_videos_iter{iter_num:06}/rgb/{pred_video_num:04}.mp4'
...
def start_qa(pred_train_dirpath: Path, database_dirpath: Path, resolution_suffix, downsampling_factor: int):
    ...
    for iter_num in iter_nums:
        ...
        avg_vmaf, vmaf_data = vmaf_computer.compute_avg_vmaf(vmaf_data, database_dirpath, pred_train_dirpath, iter_num,
...
def demo1():
    ...
    avg_score = start_qa(pred_train_dirpath, database_dirpath, resolution_suffix, downsampling_factor)
    return avg_score
