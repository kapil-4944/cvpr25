...
# SROCC measure between predicted depths and depths from dense input NeRF for spiral video
...
class DepthSROCC:
    ...
    def compute_avg_srocc(self, old_data: pandas.DataFrame, dense_model_dirpath: Path, pred_train_dirpath: Path,
        ...
        for scene_name in tqdm(scene_names, leave=self.verbose_log):
            ...
            for frame_num in range(gt_depth.shape[0]):
                if old_data is not None and old_data.loc[
                    ...
                    continue
                qa_score = self.compute_depth_srocc(gt_depth[frame_num], pred_depth[frame_num])
        ...
        if isinstance(avg_srocc, numpy.floating):
...
def start_qa(pred_train_dirpath: Path, database_dirpath: Path, dense_model_dirpath: Path, resolution_suffix,
    ...
    srocc_computer = DepthSROCC(videos_data)
    ...
    for iter_num in iter_nums:
        ...
        if str(iter_num) in qa_scores:
            if this_metric_name in qa_scores[str(iter_num)]:
                avg_srocc = qa_scores[str(iter_num)][this_metric_name]
                print(f'Average {this_metric_name}: {pred_train_dirpath.as_posix()} - {iter_num:06}: {avg_srocc}')
                print('Running QA again.')
        else:
            qa_scores[str(iter_num)] = {}

        srocc_data_path = pred_train_dirpath / f'quality_scores/iter{iter_num:06}/{this_metric_name}_FrameWise.csv'
        if srocc_data_path.exists():
            srocc_data = pandas.read_csv(srocc_data_path)
        else:
            srocc_data = None

        avg_srocc, srocc_data = srocc_computer.compute_avg_srocc(srocc_data, dense_model_dirpath, pred_train_dirpath, iter_num,
                                                             resolution_suffix, downsampling_factor)
        if numpy.isfinite(avg_srocc):
            avg_scores[iter_num] = avg_srocc
            qa_scores[str(iter_num)][this_metric_name] = avg_srocc
            print(f'Average {this_metric_name}: {pred_train_dirpath.as_posix()} - {iter_num:06}: {avg_srocc}')
