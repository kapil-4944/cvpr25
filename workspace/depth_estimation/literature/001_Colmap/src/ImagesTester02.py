...
# Predicts sparse depth from the given images and poses without camera pose information
# Extended from ImagesTester01.py to reconstruct without camera pose information
...
class ColmapTester:
    ...
    def save_tmp_data(self, image_paths: List[Path]):
    ...
    def run_colmap(self):
        ...
        # cmd = f'colmap mapper --database_path {self.db_path.as_posix()} --image_path {self.images_dirpath.as_posix()} --export_path {self.sparse_dirpath.as_posix()} --Mapper.tri_ignore_two_view_tracks 0 --Mapper.num_threads 16 --Mapper.init_min_tri_angle 4 --Mapper.multiple_models 0 --Mapper.extract_colors 0'
        print(cmd)
        os.system(cmd)
    ...
    def estimate_sparse_depth(self, image_paths: List[Path], extrinsics: numpy.ndarray, intrinsics: numpy.ndarray, resolution: tuple = None):
        self.clean_tmp_dir()
        self.save_tmp_data(image_paths)
        self.run_colmap()
