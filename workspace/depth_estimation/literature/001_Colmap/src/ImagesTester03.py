...
# Predicts sparse depth from the given images and poses. Supports different camera intrinsics
# Modified from ImagesTester01.py to support different intrinsics for different cameras
...
class ColmapTester:
    def __init__(self, tmp_dirpath: Path):
        ...
        self.sparse_dirpath0 = self.sparse_dirpath / '0'
        self.camera_path = self.sparse_dirpath0 / 'cameras.txt'
        self.images_path = self.sparse_dirpath0 / 'images.txt'
        self.points_path = self.sparse_dirpath0 / 'points3D.txt'
        return
    ...
    def save_tmp_data(self, image_paths: List[Path], intrinsics: numpy.ndarray, resolution: tuple = None):
        self.sparse_dirpath0.mkdir(parents=True, exist_ok=True)
        ...
        camera_data_str = ''
        camera_data = {}
        unique_intrinsics = numpy.unique(intrinsics, axis=0)
        for cam_idx, intrinsic in enumerate(unique_intrinsics):
            camera_id = cam_idx + 1
            camera_data_str += f'{camera_id} FULL_OPENCV {w} {h} {intrinsic[0,0]} {intrinsic[1,1]} {intrinsic[0,2]} {intrinsic[1,2]} 0 0 0 0 0 0 0 0 \n'
            camera_data[camera_id] = intrinsic
        with open(self.camera_path.as_posix(), 'w') as camera_file:
            camera_file.writelines(camera_data_str)
    ...
    def run_colmap(self, camera_data, extrinsics: numpy.ndarray):
        ...
        for camera_id, intrinsic in camera_data.items():
        ...
        cmd = f'colmap point_triangulator --database_path {self.db_path.as_posix()} --image_path {self.images_dirpath.as_posix()} --input_path {self.sparse_dirpath0.as_posix()} --output_path {self.sparse_dirpath0.as_posix()} --Mapper.tri_ignore_two_view_tracks 0 --Mapper.num_threads 16 --Mapper.init_min_tri_angle 4 --Mapper.multiple_models 0 --Mapper.extract_colors 0'
        # cmd = f'colmap point_triangulator --database_path {self.db_path.as_posix()} --image_path {self.images_dirpath.as_posix()} --import_path {self.sparse_dirpath0.as_posix()} --export_path {self.sparse_dirpath0.as_posix()} --Mapper.tri_ignore_two_view_tracks 0 --Mapper.num_threads 16 --Mapper.init_min_tri_angle 4 --Mapper.multiple_models 0 --Mapper.extract_colors 0'
        print(cmd)
        os.system(cmd)
    
        cmd = f'colmap model_converter --input_path {self.sparse_dirpath0.as_posix()} --output_path {self.sparse_dirpath0.as_posix()} --output_type TXT'
        print(cmd)
        os.system(cmd)
        return
    ...
    def load_colmap_data(self):
        camdata = read_model.read_cameras_binary((self.sparse_dirpath0 / 'cameras.bin').as_posix())
        ...
        imdata = read_model.read_images_binary((self.sparse_dirpath0 / 'images.bin'))
        ...
        pts3d = read_model.read_points3d_binary((self.sparse_dirpath0 / 'points3D.bin'))
    ...
    def compute_colmap_depth(self):
        if not (self.sparse_dirpath0 / 'images.bin').exists():
            return None, None

        images = read_images_binary((self.sparse_dirpath0 / 'images.bin').as_posix())
        points = read_points3d_binary((self.sparse_dirpath0 / 'points3D.bin').as_posix())
