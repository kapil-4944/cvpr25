...
from .regularization import (
    PlaneTV, TimeSmoothness, HistogramLoss, L1TimePlanes, DistortionLoss, SparseFlowLoss, DenseFlowLoss, WeightedProjectionLoss,WindowFlowSmoothness, SparseDepthLoss,FlowConsistencyLoss, FlowConsistencyLoss02
)
...
class VideoTrainer(BaseTrainer):
    ...
    def eval_step(self, data, **kwargs) -> MutableMapping[str, torch.Tensor]:
        ...
        flow_visulization = kwargs.get('flow_prediction', False)                           #TODO Kapil : added line 72 and 92-94 for flow visualization
    ...
    def validate(self):
        ...
        flow_prediction_frame  = self.extra_args['flow_prediction_frame']
        ...
        for img_idx, data in enumerate(dataset):
            ...
            if pred_frame_path.exists() and pred_depth_path.exists():
                ...
                if img_idx == flow_prediction_frame:
                    flow_validation = True
                    preds = self.eval_step(data, video_index=video_index, flow_prediction=flow_validation)
                else:
                    flow_validation = False
                    preds = self.eval_step(data, video_index=video_index, flow_prediction=flow_validation)
            out_metrics, out_img, out_depth = self.evaluate_metrics(
                data["imgs"], preds, dset=dataset, img_idx=img_idx, video_indx=video_num, name=None,
                save_outputs=self.save_outputs, flow_validation=flow_validation, same_frame_flow_validation=True)
    ...
    def get_regularizers(self, **kwargs):
        if 'weighted_projection_loss_weight' not in self.extra_args:
            if isinstance(self.extra_args["consistency_optim_type"], str):
                ...
                return [
                    ...
                    DenseFlowLoss(kwargs.get('dense_flow_loss_weight', 0.0),
                                  ...
                                  df_stop_gradient_weights=kwargs.get('dense_flow_loss_stop_gradient_weights', False)),
                    # SparseDepthLoss(kwargs.get('sparse_depth_loss_weight', 0.0)),
                    # FlowConsistencyLoss(kwargs.get('flow_consistency_weight', 0.0), fc_2d=kwargs['flow_consistency_2d'],
                    #                     fc_stop_gradient_weights=kwargs['flow_consistency_stop_gradient_weights']),
                    FlowConsistencyLoss02(kwargs.get('flow_consistency_weight', 0.0),
                                          fcap=kwargs.get('flow_consistency_average_point'),
                                          fc_stop_gradient_weights=kwargs['flow_consistency_stop_gradient_weights']),
                    WindowFlowSmoothness(kwargs.get('sf_neighbours_smoothness_loss_weight', 0.0),
                                        fcap=True,
                                        fc_stop_gradient_weights=True),
            ...
            if isinstance(self.extra_args["consistency_optim_type"], str):
                ...
                DenseFlowLoss(kwargs.get('dense_flow_loss_weight', 0.0),
                              ...
                              df_stop_gradient_weights=kwargs.get('dense_flow_loss_stop_gradient_weights', False)),
                # SparseDepthLoss(kwargs.get('sparse_depth_loss_weight', 0.0)),
                WeightedProjectionLoss(kwargs.get('weighted_projection_loss_weight', 0.0),
                                    fcap=kwargs.get('flow_consistency_average_point'),
                                    fc_stop_gradient_weights=kwargs['flow_consistency_stop_gradient_weights']),
                ]
            else:
    ...
    def consistency_regulizerss(self, **kwargs):
        return [
            FlowConsistencyLoss02(kwargs.get('flow_consistency_weight', 0.0),
                                  fcap=kwargs.get('flow_consistency_average_point'),
                                  fc_stop_gradient_weights=kwargs['flow_consistency_stop_gradient_weights']),
