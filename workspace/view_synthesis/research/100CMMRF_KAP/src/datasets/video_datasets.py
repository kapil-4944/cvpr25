...
class Video360Dataset(BaseDataset):
    ...
    def __init__(self,
        ...
        self.sparse_flow_smoothness_window_size = 4
    ...
    def enable_isg(self):
        raise RuntimeError("ISG is disabled manually by SNB.")
        self.isg = True
        self.ist = False
    ...
    @staticmethod
    def sf_neighbouring_points(x1, y1, x2, y2, neighbours, neighbours_direction):
        """
        :param x1:
        :param y1:
        :param x2:
        ...
        :return:
        """
        # all neighbouring points in the window_size x window_size window
        if neighbours == 4:
            if neighbours_direction == 'right':
                x1_window = x1 + 1
                y1_window = y1
                x2_window = x2 + 1
                y2_window = y2
            elif neighbours_direction == 'left':
                x1_window = x1 - 1
                y1_window = y1
                x2_window = x2 - 1
                y2_window = y2
            elif neighbours_direction == 'up':
                x1_window = x1
                y1_window = y1 - 1
                x2_window = x2
                y2_window = y2 - 1

            elif neighbours_direction == 'down':
                x1_window = x1
                y1_window = y1 + 1
                x2_window = x2
                y2_window = y2 + 1
            elif neighbours_direction == 'all':
                x1_window = numpy.concatenate([x1 + 1, x1 - 1, x1, x1],axis=0)
                y1_window = numpy.concatenate([y1, y1, y1 + 1, y1 - 1],axis=0)
                x2_window = numpy.concatenate([x2 + 1, x2 - 1, x2, x2],axis=0)
                y2_window = numpy.concatenate([y2, y2, y2 + 1, y2 - 1],axis=0)
            else:
                raise ValueError('Invalid neighbours_direction')
        elif neighbours == 8:
            if neighbours_direction == 'right':
                x1_window = x1 + 1
                y1_window = y1
                ...
                x2_window = x2
                y2_window = y2 + 1
            elif neighbours_direction == 'up_right':
                x1_window = x1 + 1
                y1_window = y1 - 1
                x2_window = x2 + 1
                y2_window = y2 - 1
            elif neighbours_direction == 'up_left':
                x1_window = x1 - 1
                y1_window = y1 - 1
                x2_window = x2 - 1
                y2_window = y2 - 1
            elif neighbours_direction == 'down_right':
                x1_window = x1 + 1
                y1_window = y1 + 1
                x2_window = x2 + 1
                y2_window = y2 + 1
            elif neighbours_direction == 'down_left':
                x1_window = x1 - 1
                y1_window = y1 + 1
                x2_window = x2 - 1
                y2_window = y2 + 1
            elif neighbours_direction == 'all':
                x1_window = numpy.concatenate([x1 + 1, x1 - 1, x1, x1, x1 + 1, x1 - 1, x1 + 1, x1 - 1], axis=0)
                y1_window = numpy.concatenate([y1, y1,  y1 - 1, y1 + 1, y1 - 1, y1 - 1, y1 + 1, y1 + 1],axis=0)
                x2_window = numpy.concatenate([x2 + 1, x2 - 1, x2, x2, x2 + 1, x2 - 1, x2 + 1, x2 - 1],axis=0)
                y2_window = numpy.concatenate([y2, y2, y2 - 1, y2 + 1, y2 - 1, y2 - 1, y2 + 1, y2 + 1],axis=0)
            else:
                raise ValueError('Invalid neighbours_direction')
        else:
            raise ValueError('NEIGHBOUR CONDITION NOT IMPLEMENTED')

        return x1_window, y1_window, x2_window, y2_window
    ...
    def add_sparse_flow_data(self, return_dict: dict, device):
        ...
        if self.sparse_flow_smoothness_window_size != 0:
            # points surrounding the sparse flow points with window size self.sparse_flow_smoothness_window_size
            random_neighbours = ['right', 'left', 'up', 'down']
            rand_i = numpy.random.choice(random_neighbours,1)[0]
            x1_sf_window, y1_sf_window, x2_sf_window, y2_sf_window = self.sf_neighbouring_points(x1_sf, y1_sf, x2_sf, y2_sf, 4, 'all')
            video1_num_sf_window = video1_num_sf.repeat(x1_sf_window.shape[0]//video1_num_sf.shape[0])
            frame1_num_sf_window = frame1_num_sf.repeat(x1_sf_window.shape[0]//video1_num_sf.shape[0])
            video2_num_sf_window = video2_num_sf.repeat(x2_sf_window.shape[0]//video2_num_sf.shape[0])
            frame2_num_sf_window = frame2_num_sf.repeat(x2_sf_window.shape[0]//video2_num_sf.shape[0])
            # TODO : make video1_num_sf_window, video2_num_sf_window to the same size as x1_sf_window, for more then one neighbour case
            video1_index_sf_window = numpy.where(self.video_ids == video1_num_sf_window[:, None])[1]
            video2_index_sf_window = numpy.where(self.video_ids == video2_num_sf_window[:, None])[1]
            image_id1_sf_window = video1_index_sf_window * self.num_frames_per_camera + frame1_num_sf_window.astype('int64')
            image_id2_sf_window = video2_index_sf_window * self.num_frames_per_camera + frame2_num_sf_window.astype('int64')
            index1_sf_window = image_id1_sf_window * h * w + y1_sf_window.astype('int64') * w + x1_sf_window.astype('int64')
            index2_sf_window = image_id2_sf_window * h * w + y2_sf_window.astype('int64') * w + x2_sf_window.astype('int64')
            timestamps1_sf_window = self.timestamps[index1_sf_window]  # (num_rays or 1, )
            timestamps2_sf_window = self.timestamps[index2_sf_window]
            self.concat_dict_element(return_dict, 'timestamps', timestamps1_sf_window)
            self.concat_dict_element(return_dict, 'timestamps', timestamps2_sf_window)
            self.concat_dict_element(return_dict, 'timestamps2', timestamps2_sf_window)
            self.concat_dict_element(return_dict, 'timestamps2', timestamps1_sf_window)

            camera_id1_sf_window = image_id1_sf_window // self.num_frames_per_camera  # (num_rays)
            camera_id2_sf_window = image_id2_sf_window // self.num_frames_per_camera
            near_fars1_sf_window = self.per_cam_near_fars[camera_id1_sf_window, :]
            near_fars2_sf_window = self.per_cam_near_fars[camera_id2_sf_window, :]
            self.concat_dict_element(return_dict, 'near_fars', near_fars1_sf_window)
            self.concat_dict_element(return_dict, 'near_fars', near_fars2_sf_window)
            imgs1_sf_window = (self.imgs[index1_sf_window] / 255.0).view(-1, self.imgs.shape[-1])
            imgs2_sf_window = (self.imgs[index2_sf_window] / 255.0).view(-1, self.imgs.shape[-1])
            # Alpha compositing
            if imgs1_sf_window is not None and imgs1_sf_window.shape[-1] == 4:
                imgs1_sf_window = imgs1_sf_window[:, :3] * imgs1_sf_window[:, 3:] + return_dict['bg_color'] * (1.0 - imgs1_sf_window[:, 3:])
            if imgs2_sf_window is not None and imgs2_sf_window.shape[-1] == 4:
                imgs2_sf_window = imgs2_sf_window[:, :3] * imgs2_sf_window[:, 3:] + return_dict['bg_color'] * (1.0 - imgs2_sf_window[:, 3:])
            self.concat_dict_element(return_dict, 'imgs', imgs1_sf_window)
            self.concat_dict_element(return_dict, 'imgs', imgs2_sf_window)
            c2w1_sf_window = self.poses[image_id1_sf_window]  # [num_rays or 1, 3, 4]
            c2w2_sf_window = self.poses[image_id2_sf_window]
            video1_num_sf_window,frame1_num_sf_window, video2_num_sf_window, frame2_num_sf_window = map(lambda x: torch.from_numpy(x).float().to(device), [video1_num_sf_window, frame1_num_sf_window, video2_num_sf_window, frame2_num_sf_window])
            x1_sf_window, y1_sf_window, x2_sf_window, y2_sf_window = map(lambda x: torch.from_numpy(x).float().to(device), [x1_sf_window, y1_sf_window, x2_sf_window, y2_sf_window])
            camera_dirs1_sf_window = stack_camera_dirs(x1_sf_window, y1_sf_window, self.intrinsics[image_id1_sf_window], True)  # [num_rays, 3]
            camera_dirs2_sf_window = stack_camera_dirs(x2_sf_window, y2_sf_window, self.intrinsics[image_id2_sf_window], True)
            rays_o1_sf_window, rays_d1_sf_window = get_rays(camera_dirs1_sf_window, c2w1_sf_window, ndc=self.is_ndc, ndc_near=1.0, intrinsics=self.intrinsics[image_id1_sf_window], resolution=self.resolution, normalize_rd=True)  # [num_rays, 3]
            rays_o2_sf_window, rays_d2_sf_window = get_rays(camera_dirs2_sf_window, c2w2_sf_window, ndc=self.is_ndc, ndc_near=1.0, intrinsics=self.intrinsics[image_id2_sf_window], resolution=self.resolution, normalize_rd=True)
            self.concat_dict_element(return_dict, 'rays_o', rays_o1_sf_window)
            self.concat_dict_element(return_dict, 'rays_o', rays_o2_sf_window)
            self.concat_dict_element(return_dict, 'rays_d', rays_d1_sf_window)
            self.concat_dict_element(return_dict, 'rays_d', rays_d2_sf_window)

            extrinsics_last_row = torch.Tensor([0, 0, 0, 1]).reshape(1, 1, 4).repeat([video1_num_sf_window.shape[0], 1, 1]).to(c2w1_sf_window)
            extrinsics1_c2w_sf_window = torch.cat([c2w1_sf_window, extrinsics_last_row], dim=1)
            extrinsics2_c2w_sf_window = torch.cat([c2w2_sf_window, extrinsics_last_row], dim=1)
            return_dict['extrinsics1_c2w_neighbours'] = extrinsics1_c2w_sf_window
            return_dict['extrinsics2_c2w_neighbours'] = extrinsics2_c2w_sf_window
            # self.concat_dict_element(return_dict, 'extrinsics1_c2w_neighbours', extrinsics1_c2w_sf_window)
            self.concat_dict_element(return_dict, 'extrinsics1_c2w_neighbours', extrinsics2_c2w_sf_window)
            # self.concat_dict_element(return_dict, 'extrinsics2_c2w_neighbours', extrinsics2_c2w_sf_window)
            self.concat_dict_element(return_dict, 'extrinsics2_c2w_neighbours', extrinsics1_c2w_sf_window)
            # self.concat_dict_element(return_dict, 'sparse_flow_mask_neighbours', torch.ones_like(video1_num_sf_window, dtype=bool))
            # self.concat_dict_element(return_dict, 'sparse_flow_mask_neighbours', torch.ones_like(video2_num_sf_window, dtype=bool))
            sf_mask1_window = torch.zeros_like(video1_num_sf_window, dtype=bool)
            sf_mask2_window = torch.zeros_like(video2_num_sf_window, dtype=bool)
            return_dict['sparse_flow_mask_neighbours'] = sf_mask1_window
            return_dict['video1_num_neighbours'] = video1_num_sf_window
            return_dict['frame1_num_neighbours'] = frame1_num_sf_window
            return_dict['video2_num_neighbours'] = video2_num_sf_window
            return_dict['frame2_num_neighbours'] = frame2_num_sf_window
            return_dict['y1_neighbours'] = y1_sf_window
            return_dict['x1_neighbours'] = x1_sf_window
            return_dict['y2_neighbours'] = y2_sf_window
            return_dict['x2_neighbours'] = x2_sf_window
            # return_dict['t_prime'] = timestamps2_sf_window
            # self.concat_dict_element(return_dict, 't_prime', timestamps1_sf_window)
            # self.concat_dict_element(return_dict, 'sparse_flow_mask_neighbours', sf_mask1_window)
            self.concat_dict_element(return_dict, 'sparse_flow_mask_neighbours', sf_mask2_window)
            # self.concat_dict_element(return_dict, 'video1_num_neighbours', video1_num_sf_window)
            self.concat_dict_element(return_dict, 'video1_num_neighbours', video2_num_sf_window)
            # self.concat_dict_element(return_dict, 'frame1_num_neighbours', frame1_num_sf_window)
            self.concat_dict_element(return_dict, 'frame1_num_neighbours', frame2_num_sf_window)
            # self.concat_dict_element(return_dict, 'video2_num_neighbours', video2_num_sf_window)
            self.concat_dict_element(return_dict, 'video2_num_neighbours', video1_num_sf_window)
            # self.concat_dict_element(return_dict, 'frame2_num_neighbours', frame2_num_sf_window)
            self.concat_dict_element(return_dict, 'frame2_num_neighbours', frame1_num_sf_window)
            # self.concat_dict_element(return_dict, 'y1_neighbours', y1_sf_window)
            self.concat_dict_element(return_dict, 'y1_neighbours', y2_sf_window)
            # self.concat_dict_element(return_dict, 'x1_neighbours', x1_sf_window)
            self.concat_dict_element(return_dict, 'x1_neighbours', x2_sf_window)
            # self.concat_dict_element(return_dict, 'y2_neighbours', y2_sf_window)
            self.concat_dict_element(return_dict, 'y2_neighbours', y1_sf_window)
            # self.concat_dict_element(return_dict, 'x2_neighbours', x2_sf_window)
            self.concat_dict_element(return_dict, 'x2_neighbours', x1_sf_window)

            self.concat_dict_element(return_dict,'extrinsics1_c2w',extrinsics1_c2w_sf_window)
            self.concat_dict_element(return_dict,'extrinsics1_c2w',extrinsics2_c2w_sf_window)
            self.concat_dict_element(return_dict,'extrinsics2_c2w',extrinsics2_c2w_sf_window)
            self.concat_dict_element(return_dict,'extrinsics2_c2w',extrinsics1_c2w_sf_window)
            self.concat_dict_element(return_dict, 'x1', x1_sf_window)
            self.concat_dict_element(return_dict, 'x1', x2_sf_window)
            self.concat_dict_element(return_dict, 'y1', y1_sf_window)
            self.concat_dict_element(return_dict, 'y1', y2_sf_window)
            self.concat_dict_element(return_dict, 'x2', x2_sf_window)
            self.concat_dict_element(return_dict, 'x2', x1_sf_window)
            self.concat_dict_element(return_dict, 'y2', y2_sf_window)
            self.concat_dict_element(return_dict, 'y2', y1_sf_window)
        ...
        # TODO: last UPDATED BY KAPIL ON JUNE 9
        self.sparse_flow_smoothness_window_size = 4

        if 'sparse_flow_mask_neighbours' in return_dict:
            sf_mask1_window = torch.zeros_like(video1_num_sf_window, dtype=bool)
            sf_mask2_window = torch.zeros_like(video2_num_sf_window, dtype=bool)
            self.concat_dict_element(return_dict, 'sparse_flow_mask', sf_mask1_window)
            self.concat_dict_element(return_dict, 'sparse_flow_mask', sf_mask2_window)
        ...
        if 'dense_flow_mask' in return_dict:
            if 'sparse_flow_mask_neighbours' in return_dict:
                a= True
                df_mask1_window = torch.zeros_like(video1_num_sf_window, dtype=bool)
                df_mask2_window = torch.zeros_like(video2_num_sf_window, dtype=bool)
                self.concat_dict_element(return_dict, 'dense_flow_mask', df_mask1_window)
                self.concat_dict_element(return_dict, 'dense_flow_mask', df_mask2_window)
